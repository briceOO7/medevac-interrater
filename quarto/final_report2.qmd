...
# Primary model: vignette type + confidence (within/between) + experience + rural experience
# Note: Excludes Special Considerations vignettes (n=3) as sensitivity analysis
# Physician random effect removed: all between-physician variation explained by fixed effects
model_primary <- glmer(chose_medevac ~ medevac_status_label + 
                  confidence_within +
                  confidence_between +
                  years_experience_centered +
                  rural_experience +
                  (1 | question),  # Only vignette random effect
                data = data_model,
...
total_var <- var_vignette + var_residual
...
We use a mixed-effects logistic regression model to account for the nested structure of our data: multiple responses from each physician across multiple vignettes. Standard logistic regression would violate the assumption of independent observations, as responses to the same vignette are correlated across physicians. The GLMM approach includes random intercepts for vignettes and physician-level fixed effects (average confidence, experience, practice location), allowing us to: (1) obtain valid standard errors and p-values that account for vignette clustering, (2) separate within-physician effects (e.g., being more confident than usual on a specific vignette) from between-physician effects (e.g., generally confident physicians), and (3) quantify how much variation exists at the vignette level beyond what's explained by vignette type and physician characteristics. Note that physician random effects were not included as all between-physician variation was explained by the measured physician-level predictors.
...
fixed_table <- data.frame(
  ...
  OR = c(NA, 1.00, or[2:3], NA, NA, or[6], or[5], or[7], NA, NA, or[4]),
  CI_lower = c(NA, NA, or_lower[2:3], NA, NA, or_lower[6], or_lower[5], or_lower[7], NA, NA, or_lower[4]),
  CI_upper = c(NA, NA, or_upper[2:3], NA, NA, or_upper[6], or_upper[5], or_upper[7], NA, NA, or_upper[4]),
  p_value_raw = c(NA, NA, p_values[2:3], NA, NA, p_values[6], p_values[5], p_values[7], NA, NA, p_values[4])
) %>%
  mutate(
...
- **Within-Person Confidence**: OR = `r sprintf("%.2f", or[4])` per 1-point increase (p = `r ifelse(p_values[4] < 0.001, "<0.001", sprintf("%.3f", p_values[4]))`) - Being more confident than usual increases medevac likelihood

**Non-Significant Predictors (p ≥ 0.05):**

- **Between-Person Confidence**: OR = `r sprintf("%.2f", or[5])` (p = `r sprintf("%.3f", p_values[5])`) - Overall confidence level not associated with medevac propensity
- **Years of Experience**: OR = `r sprintf("%.2f", or[6])` per year (p = `r sprintf("%.3f", p_values[6])`) - Experience not significantly associated with utilization
- **Rural Experience**: OR = `r sprintf("%.2f", or[7])` (p = `r sprintf("%.3f", p_values[7])`) - Having worked at regional/rural sites not significantly associated with medevac utilization
...
kable(variance_table,
  ...
  footnote(general = "Note: Physician random effects not included. All between-physician variation is explained by measured physician-level predictors (average confidence, years of experience, rural practice experience).",
           general_title = "")
```

**Key Findings:**

- **Vignette ICC**: `r sprintf("%.1f%%", 100*icc_vignette)` of residual variation is between vignettes (after accounting for vignette type)
- **Vignette MOR**: `r sprintf("%.2f", mor_vignette)` - Two randomly selected vignettes of the same type differ by at most this odds ratio
...
- **Rural experience**: Physicians with rural/regional practice experience do not show significantly different medevac utilization compared to those with referral hub experience only (OR = `r sprintf("%.2f", exp(fixed_effects["rural_experienceYes"]))`, 95% CI: `r sprintf("%.2f-%.2f", or_lower["rural_experienceYes"], or_upper["rural_experienceYes"])`, p = `r sprintf("%.3f", summary(model_primary)$coefficients["rural_experienceYes", "Pr(>|z|)"])`).

- **Physician variation is fully explained by measured predictors**: All between-physician variation in medevac utilization is accounted for by their average confidence level, years of experience, and practice location. No residual "physician effect" or unmeasured risk tolerance remains after controlling for these characteristics. This indicates that physician decision-making is systematic and explainable rather than idiosyncratic.
...
**2. Physician variation is fully explained**
   - All between-physician variation accounted for by measured characteristics
   - No residual "physician effect" or unmeasured risk tolerance
   - Systematic, explainable decision-making patterns

**3. Vignette variation remains substantial**
   - ICC (Vignette) = `r sprintf("%.1f%%", icc_vignette * 100)` of residual variation between vignettes
   - MOR = `r sprintf("%.2f", mor_vignette)` - substantial differences between scenarios of same type
   - Clinical nuances within vignette categories drive remaining variation

## Methodological Strengths

- **Mixed-effects framework** accounts for vignette clustering
- **Within-between decomposition** separates case-specific from physician-level confidence effects
- **Explicit appropriateness modeling** moves beyond raw agreement
- **Physician-level predictors** explain all between-physician variation
- **Variance decomposition** quantifies sources of variation

## Implications

This analysis demonstrates that physicians respond appropriately to clinical scenarios in a systematic, explainable manner. All variation in medevac decision-making is attributable to:
