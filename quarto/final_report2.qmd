...
data <- data %>%
  mutate(
    ...
    # Set factor levels with Never Medevac as reference (SIMPLIFIED - 3 categories)
...
- **Possible Medevac** (n=12): Medevac may or may not be appropriate depending on clinical judgment
...
normative_counts <- data %>%
  ...
  arrange(factor(medevac_status_label, 
                 levels = c("Never Medevac", "Possible Medevac", "Always Medevac")))
...
pred_logodds <- c(
  ...
  fixed_effects_fig3[1] + fixed_effects_fig3[3]   # Always Medevac
)
...
ggplot(marginal_plot_data, aes(x = category_num, y = predicted)) +
  ...
  scale_x_continuous(breaks = 1:3, labels = categories) +
...
# Primary model: vignette type + confidence + experience + practice location (no interaction)
...
fixed_table <- data.frame(
  ...
  OR = c(NA, 1.00, or[2:3], NA, NA, or[4:5], NA, NA, 1.00, or[6:7]),
  CI_lower = c(NA, NA, or_lower[2:3], NA, NA, or_lower[4:5], NA, NA, NA, or_lower[6:7]),
  CI_upper = c(NA, NA, or_upper[2:3], NA, NA, or_upper[4:5], NA, NA, NA, or_upper[6:7]),
  p_value_raw = c(NA, NA, p_values[2:3], NA, NA, p_values[4:5], NA, NA, NA, p_values[6:7])
) %>%
  mutate(
