...
# Assign quartile labels with sequential numbers for unique identification
physician_letter_map <- physician_exp %>%
  ...
  mutate(
    ...
    physician_letter_full = paste0(quartile, quartile_num),  # Unique ID: A1, A2, etc.
    physician_letter_display = as.character(quartile)  # Display label: A, B, etc.
  ) %>%
  ungroup() %>%
  select(physician_id, physician_letter_full, physician_letter_display, quartile, years_experience)
...
physician_decisions <- data %>%
  ...
  group_by(physician_letter_full) %>%
  summarise(
...
physician_long <- physician_decisions %>%
  select(Physician = physician_letter_full, Medevac = pct_medevac, 
