...
# Re-level factor with Never Medevac as reference (all 4 categories)
data <- data %>%
  mutate(
    medevac_status_label = factor(medevac_status_label,
...
fixed_table <- data.frame(
  ...
  OR = c(NA, 1.00, or[2:4], NA, NA, or[7], or[6], or[8], NA, NA, or[5]),
  CI_lower = c(NA, NA, or_lower[2:4], NA, NA, or_lower[7], or_lower[6], or_lower[8], NA, NA, or_lower[5]),
  CI_upper = c(NA, NA, or_upper[2:4], NA, NA, or_upper[7], or_upper[6], or_upper[8], NA, NA, or_upper[5]),
  p_value_raw = c(NA, NA, p_values[2:4], NA, NA, p_values[7], p_values[6], p_values[8], NA, NA, p_values[5])
) %>%
  mutate(
...
- **Vignette Type**: Compared to Never Medevac (reference):
  ...
  - Always Medevac: OR = `r sprintf("%.2f", or[4])` (p < 0.001)
...
- **Rural Experience**: OR = `r sprintf("%.2f", or[8])` (p = `r sprintf("%.3f", p_values[8])`) - Having worked at regional/rural sites not significantly associated with medevac utilization
