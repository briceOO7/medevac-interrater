...
## Figure 2: Confidence and Agreement by Vignette

```{r figure2-confidence-agreement}
...
ggplot(vignette_conf_agree, aes(x = agreement_pct, y = mean_confidence, 
  ...
  labs(
    title = "Figure 2: Confidence and Agreement by Vignette",
...
## Figure 3: Medevac Utilization Rates by Vignette Type

```{r figure3-marginal-probs}
#| fig-height: 6
#| fig-width: 10
...
model_for_fig3 <- glmer(chose_medevac ~ medevac_status_label + 
                  confidence_within +
                  confidence_between +
...
# Get fixed effects
fixed_effects_fig3 <- fixef(model_for_fig3)
vcov_mat_fig3 <- vcov(model_for_fig3)
...
pred_logodds <- c(
  fixed_effects_fig3[1],  # Never Medevac (reference)
  fixed_effects_fig3[1] + fixed_effects_fig3[2],  # Possible Medevac
  fixed_effects_fig3[1] + fixed_effects_fig3[3],  # Special Considerations
  fixed_effects_fig3[1] + fixed_effects_fig3[4]   # Always Medevac
)

# Calculate standard errors using delta method
se_logodds <- c(
  sqrt(vcov_mat_fig3[1,1]),
  sqrt(vcov_mat_fig3[1,1] + vcov_mat_fig3[2,2] + 2*vcov_mat_fig3[1,2]),
  sqrt(vcov_mat_fig3[1,1] + vcov_mat_fig3[3,3] + 2*vcov_mat_fig3[1,3]),
  sqrt(vcov_mat_fig3[1,1] + vcov_mat_fig3[4,4] + 2*vcov_mat_fig3[1,4])
)
...
ggplot(marginal_plot_data, aes(x = category_num, y = predicted)) +
  ...
  labs(
    title = "Figure 3: Medevac Utilization Rates by Vignette Type",
...
### Summary Statistics by Vignette Type
...
vignette_summary <- data %>%
  ...
  mutate(
    ...
    confidence_lower = mean_confidence - 1.96 * se_confidence,
    confidence_upper = mean_confidence + 1.96 * se_confidence
  )

# Panel A: Agreement
p_agreement <- ggplot(vignette_summary, aes(x = medevac_status_label, y = mean_agreement)) +
  geom_col(fill = "#3498DB", alpha = 0.7) +
  geom_errorbar(aes(ymin = agreement_lower, ymax = agreement_upper), 
                width = 0.3, size = 1) +
  geom_text(aes(label = sprintf("%.1f%%", mean_agreement)),
            vjust = -1.5, size = 4, fontface = "bold") +
  labs(
    title = "A. Inter-Rater Agreement",
    x = NULL,
    y = "Agreement (%)"
  ) +
...
# Panel B: Confidence
p_confidence <- ggplot(vignette_summary, aes(x = medevac_status_label, y = mean_confidence)) +
  geom_col(fill = "#E67E22", alpha = 0.7) +
  geom_errorbar(aes(ymin = confidence_lower, ymax = confidence_upper), 
                width = 0.3, size = 1) +
  geom_text(aes(label = sprintf("%.1f", mean_confidence)),
            vjust = -1.5, size = 4, fontface = "bold") +
  labs(
    title = "B. Mean Confidence",
    x = "Vignette Type",
    y = "Confidence (1-10 scale)"
  ) +
...
# Combine panels
grid.arrange(p_agreement, p_confidence, ncol = 1)
```

**Key Patterns:**
- **Agreement**: Higher agreement on clear-cut cases (Always/Never Medevac)
- **Confidence**: Physicians are most confident on cases where medevac is clearly indicated (Always Medevac)
