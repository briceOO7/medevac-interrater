...
# Load rural experience data
rural_experience <- read.csv(here::here("data", "rural_experience.csv"),
                             stringsAsFactors = FALSE)
...
data <- data %>%
  ...
  left_join(physician_experience %>% select(physician_id, years_experience),
            by = "physician_id") %>%
  left_join(rural_experience %>% select(physician_id, rural_experience),
            by = "physician_id")
  ...
  mutate(
    ...
    medevac_status_label = factor(medevac_status_label,
                                   levels = c("Never Medevac",
                                            ...
                                            "Always Medevac")),
    # Factor rural experience
    rural_experience = factor(rural_experience, levels = c("No", "Yes")),
...
physician_summary <- data %>%
  ...
  summarise(
    ...
    rural_experience = first(rural_experience),
    n_responses = n(),
    n_medevac = sum(decision == "Medevac"),
    pct_medevac = 100 * mean(decision == "Medevac")
  )

# Calculate rural experience counts
rural_exp_summary <- physician_summary %>%
  count(rural_experience)

n_rural_yes <- rural_exp_summary %>% filter(rural_experience == "Yes") %>% pull(n)
n_rural_no <- rural_exp_summary %>% filter(rural_experience == "No") %>% pull(n)
n_physicians <- length(unique(data$physician_id))
pct_rural_yes <- 100 * n_rural_yes / n_physicians
pct_rural_no <- 100 * n_rural_no / n_physicians
...
vals <- c(
  ...
  "",
  "",
  sprintf("%d (%.0f%%)", n_rural_yes, pct_rural_yes),
  sprintf("%d (%.0f%%)", n_rural_no, pct_rural_no),
  "",
  "",
...
# Primary model: vignette type + confidence + experience + rural experience (no interaction)
...
fixed_table <- data.frame(
  Predictor = c(
    ...
    "",
    "**Practice Setting**",
    "  Rural Experience: No (reference)",
    "  Rural Experience: Yes"
  ),
  OR = c(NA, 1.00, or[2:5], NA, NA, or[6:7], NA, NA, 1.00, or[8]),
  CI_lower = c(NA, NA, or_lower[2:5], NA, NA, or_lower[6:7], NA, NA, NA, or_lower[8]),
  CI_upper = c(NA, NA, or_upper[2:5], NA, NA, or_upper[6:7], NA, NA, NA, or_upper[8]),
  p_value_raw = c(NA, NA, p_values[2:5], NA, NA, p_values[6:7], NA, NA, NA, p_values[8])
) %>%
  mutate(
    OR_display = case_when(
      Predictor %in% c("**Vignette Type**", "", "**Decision Characteristics**", "**Practice Setting**") ~ "",
      Predictor %in% c("  Never Medevac (reference)", "  Rural Experience: No (reference)") ~ "1.00",
      !is.na(OR) ~ sprintf("%.2f", OR),
      TRUE ~ ""
    ),
    CI_display = case_when(
      Predictor %in% c("**Vignette Type**", "", "**Decision Characteristics**", "**Practice Setting**") ~ "",
      Predictor %in% c("  Never Medevac (reference)", "  Rural Experience: No (reference)") ~ "ref",
