...
- **Physician ICC**: `r sprintf("%.1f%%", 100*icc_physician)` of residual variation is between physicians (unmeasured risk tolerance)
- **Vignette ICC**: `r sprintf("%.1f%%", 100*icc_vignette)` of residual variation is between vignettes
- **Physician MOR**: `r sprintf("%.2f", mor_physician)` - Two randomly selected physicians differ by at most this odds ratio in unmeasured risk tolerance
- **Vignette MOR**: `r sprintf("%.2f", mor_vignette)` - Two randomly selected vignettes differ by at most this odds ratio
...
- **Case-specific confidence influences decisions**: Higher confidence on individual cases increases likelihood of medevac utilization (OR = `r sprintf("%.2f", exp(fixed_effects["confidence_within"]))` per 1-point increase compared to physician's average, 95% CI: `r sprintf("%.2f-%.2f", ci_conf_within[1], ci_conf_within[2])`, p = `r sprintf("%.3f", summary(model_primary)$coefficients["confidence_within", "Pr(>|z|)"])`). This represents the effect of feeling unusually confident on a specific case, independent of general confidence level.

- **Rural experience and years of practice**: Neither rural/regional practice experience (OR = `r sprintf("%.2f", exp(fixed_effects["rural_experienceYes"]))`, p = `r sprintf("%.3f", summary(model_primary)$coefficients["rural_experienceYes", "Pr(>|z|)"])`) nor years of experience (OR = `r sprintf("%.2f", exp(fixed_effects["years_experience_centered"]))`, p = `r sprintf("%.3f", summary(model_primary)$coefficients["years_experience_centered", "Pr(>|z|)"])`) show significant associations with medevac utilization after accounting for case-specific confidence and vignette type.

- **Unmeasured physician variance reflects risk tolerance**: After accounting for case-specific confidence, experience, and practice location, modest physician-level variation remains (MOR = `r sprintf("%.2f", mor_physician)`; ICC = `r sprintf("%.1f%%", 100*icc_physician)`). This residual variance may be interpreted as individual differences in risk tolerance or other unmeasured stable physician characteristics. The small magnitude indicates that physician decision-making is largely systematic and explainable.
...
**2. Case-specific confidence drives decisions** (Table 3)
   - OR = `r sprintf("%.2f", or[4])` per 1-point confidence increase
   - Being more confident than usual on a case significantly increases medevac likelihood
   - Represents within-person variation, not general confidence level

**3. Modest physician-level variation persists** (Table 4)
   - ICC (Physician) = `r sprintf("%.1f%%", icc_physician * 100)` after accounting for case-specific confidence, experience, and location
   - MOR = `r sprintf("%.2f", mor_physician)` - modest differences in unmeasured risk tolerance
   - Substantial vignette variation: ICC = `r sprintf("%.1f%%", icc_vignette * 100)`, MOR = `r sprintf("%.2f", mor_vignette)`
