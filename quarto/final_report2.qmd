...
- **Vignette Type**: Compared to Never Medevac (reference):
  - Possible Medevac: OR = `r sprintf("%.2f", or[2])` (p < 0.001)
  - Always Medevac: OR = `r sprintf("%.2f", or[3])` (p < 0.001)
- **Case-Specific Confidence**: OR = `r sprintf("%.2f", or[4])` per 1-point increase (p = `r ifelse(p_values[4] < 0.001, "<0.001", sprintf("%.3f", p_values[4]))`) - Being more confident than usual on a specific case increases medevac likelihood

**Non-Significant Predictors (p ≥ 0.05):**

- **Years of Experience**: OR = `r sprintf("%.2f", or[5])` per year (p = `r sprintf("%.3f", p_values[5])`) - Experience not significantly associated with utilization after accounting for other factors
- **Rural Experience**: OR = `r sprintf("%.2f", or[6])` (p = `r sprintf("%.3f", p_values[6])`) - Having worked at regional/rural sites not significantly associated with medevac utilization after accounting for other factors
