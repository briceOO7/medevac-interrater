...
# Include all vignette types including Special Considerations
data_model <- data %>%
  mutate(
    # Re-level factor with Never Medevac as reference
    medevac_status_label = factor(medevac_status_label,
...
# Note: Includes Special Considerations vignettes to assess physician variation in ambiguous scenarios
...
fixed_table <- data.frame(
  ...
  OR = c(NA, 1.00, or[2:4], NA, NA, or[5], NA, NA, or[6], or[7]),
  CI_lower = c(NA, NA, or_lower[2:4], NA, NA, or_lower[5], NA, NA, or_lower[6], or_lower[7]),
  CI_upper = c(NA, NA, or_upper[2:4], NA, NA, or_upper[5], NA, NA, or_upper[6], or_upper[7]),
  p_value_raw = c(NA, NA, p_values[2:4], NA, NA, p_values[5], NA, NA, p_values[6], p_values[7])
) %>%
  mutate(
    OR_display = case_when(
      Predictor %in% c("**Vignette Type**", "", "**Physician Characteristics**", "**Decision Characteristics**") ~ "",
      Predictor %in% c("  Never Medevac (reference)") ~ "1.00",
      !is.na(OR) ~ sprintf("%.2f", OR),
      TRUE ~ ""
    ),
    CI_display = case_when(
      Predictor %in% c("**Vignette Type**", "", "**Physician Characteristics**", "**Decision Characteristics**") ~ "",
      Predictor %in% c("  Never Medevac (reference)") ~ "ref",
...
**Note:** Model includes all 20 vignettes across 4 categories: Never (n=4), Possible (n=9), Special Considerations (n=3), and Always (n=4). Total: 400 responses from 20 physicians. Special Considerations vignettes represent clinically ambiguous scenarios where medevac or remain are both defensible options.

**Significant Predictors (p < 0.05):**

- **Vignette Type - Always Medevac**: OR = `r sprintf("%.2f", or[4])` (p = `r ifelse(p_values[4] < 0.001, "<0.001", sprintf("%.3f", p_values[4]))`) - Physicians are overwhelmingly more likely to choose medevac for "Always Medevac" scenarios compared to "Never Medevac" scenarios, demonstrating appropriate clinical reasoning
- **Case-Specific Confidence**: OR = `r sprintf("%.2f", or[5])` per 1-point increase (p = `r ifelse(p_values[5] < 0.001, "<0.001", sprintf("%.3f", p_values[5]))`) - Being more confident than usual on a specific case significantly increases medevac likelihood
- **Rural Experience**: OR = `r sprintf("%.2f", or[7])` (p = `r sprintf("%.3f", p_values[7])`) - Physicians with rural/regional practice experience are significantly less likely to choose medevac compared to those with referral hub experience only (OR = `r sprintf("%.2f", or[7])` represents `r sprintf("%.0f%%", (1-or[7])*100)` reduction), suggesting rural practice experience is associated with more conservative disposition decisions

**Non-Significant Predictors (p ≥ 0.05):**

- **Vignette Type - Possible Medevac**: OR = `r sprintf("%.2f", or[2])` (p = `r sprintf("%.3f", p_values[2])`) - Moderate increase compared to "Never Medevac", but does not reach statistical significance
- **Vignette Type - Special Considerations**: OR = `r sprintf("%.2f", or[3])` (p = `r sprintf("%.3f", p_values[3])`) - Intermediate odds between "Possible" and "Always" categories, marginally non-significant. These ambiguous scenarios show substantial physician variation
- **Years of Experience**: OR = `r sprintf("%.2f", or[6])` per year (p = `r sprintf("%.3f", p_values[6])`) - Years in practice not significantly associated with medevac utilization after accounting for confidence, vignette type, and practice location
