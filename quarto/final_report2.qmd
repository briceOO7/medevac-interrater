...
# Exclude Special Considerations from model (sensitivity analysis)
data_model <- data %>%
  filter(medevac_status_label != "Special Considerations") %>%
  mutate(
...
# Note: Excludes Special Considerations vignettes (n=3) as sensitivity analysis
...
fixed_table <- data.frame(
  Predictor = c(
    "**Vignette Type** (excludes Special Considerations)",
  ...
  mutate(
    OR_display = case_when(
      Predictor %in% c("**Vignette Type** (excludes Special Considerations)", "", "**Physician Characteristics**", "**Decision Characteristics**") ~ "",
...
**Note:** Special Considerations vignettes (Q16, Q17, Q18) excluded from model as sensitivity analysis. Model includes only clear Never (n=4 vignettes), Possible (n=9 vignettes), and Always (n=4 vignettes) categories. Total: 340 responses from 20 physicians across 17 vignettes.

**Significant Predictors (p < 0.05):**

- **Vignette Type - Always Medevac**: OR = `r sprintf("%.2f", or[3])` (p = `r ifelse(p_values[3] < 0.001, "<0.001", sprintf("%.3f", p_values[3]))`) - Physicians are overwhelmingly more likely to choose medevac for "Always Medevac" scenarios compared to "Never Medevac" scenarios, demonstrating appropriate clinical reasoning
...
- **Rural Experience**: OR = `r sprintf("%.2f", or[6])` (p = `r sprintf("%.3f", p_values[6])`) - Physicians with rural/regional practice experience are significantly less likely to choose medevac compared to those with referral hub experience only. This represents a 78% reduction in medevac odds, suggesting rural practice experience is associated with more conservative disposition decisions

**Non-Significant Predictors (p ≥ 0.05):**

- **Vignette Type - Possible Medevac**: OR = `r sprintf("%.2f", or[2])` (p = `r sprintf("%.3f", p_values[2])`) - While the odds ratio suggests increased likelihood compared to "Never Medevac" scenarios, this effect does not reach statistical significance, likely due to the heterogeneity within ambiguous scenarios and limited sample size
