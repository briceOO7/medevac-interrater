...
fixed_table <- data.frame(
  ...
  OR = c(NA, 1.00, or[2:3], NA, NA, or[6], or[5], NA, 1.00, or[7], or[8], NA, NA, or[4]),
  CI_lower = c(NA, NA, or_lower[2:3], NA, NA, or_lower[6], or_lower[5], NA, NA, or_lower[7], or_lower[8], NA, NA, or_lower[4]),
  CI_upper = c(NA, NA, or_upper[2:3], NA, NA, or_upper[6], or_upper[5], NA, NA, or_upper[7], or_upper[8], NA, NA, or_upper[4]),
  p_value_raw = c(NA, NA, p_values[2:3], NA, NA, p_values[6], p_values[5], NA, NA, p_values[7], p_values[8], NA, NA, p_values[4])
) %>%
  mutate(
    OR_display = case_when(
      Predictor %in% c("**Vignette Type** (excludes Special Considerations)", "", "**Physician Characteristics**", "**Decision Characteristics**", "  Practice Location:") ~ "",
...
- **Practice Location**: Regional Hub OR = `r sprintf("%.2f", or[7])` (p = `r sprintf("%.3f", p_values[7])`); Mixed OR = `r sprintf("%.2f", or[8])` (p = `r sprintf("%.3f", p_values[8])`) - No significant location effects
